...
function bbaTestApp() {
  ...
  function openFromInput(btn) {
    ...
    showFile(input.dataset.base64, input.dataset.type, input.dataset.name);
  }
  ...
  function addRow() {
    ...
    if (existingNewRow) {
      alert("Please save the current row first");
      return;
    }
  ...
  function validateMandatoryFields() {
    ...
    document
      .querySelectorAll(".mandatory-error")
      .forEach((el) => el.classList.remove("mandatory-error"));
    document.querySelectorAll(".mandatory-star").forEach((el) => el.remove());
    document
      .querySelectorAll(".mandatory-cell")
      .forEach((el) => el.classList.remove("mandatory-cell"));
    ...
    rows.forEach((row) => {
      ...
      if (dateInput && dateInput.value) {
        ...
        if (dateInput.value > today) {
          ...
          dateInput.classList.add("mandatory-error");
  ...
  function saveTable() {
    ...
    rows.forEach((row) => {
      ...
      if (bacVal && Number(bacVal) < 0) {
        alert("BAC cannot be negative");
        return;
      }
  ...
  function renderPage() {
    ...
    allData.slice(start, end).forEach((r, index) => {
      ...
      row.querySelector(".date").innerText = formatDateDDMMYYYY(r.d_test_date);
